        ...
        <motion.div variants={fadeIn} className="flex flex-col lg:flex-row gap-6 mb-12 items-center justify-between glass-card p-4 rounded-xl">
          <div className="relative w-full lg:w-96 flex-shrink-0">
            ...
            <input 
              ...
              className="w-full bg-[rgb(var(--secondary-bg))] dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg pl-10 pr-4 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:border-accent-orange transition-colors"
              value={searchQuery}
          ...
          <div className="w-full lg:w-48 flex-shrink-0">
            <select 
              className="w-full bg-[rgb(var(--secondary-bg))] dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:border-accent-orange appearance-none"
              value={activeSort}
              onChange={(e) => setActiveSort(e.target.value)}
            >
              {SORTS.map(sort => <option key={sort} className="bg-[rgb(var(--secondary-bg))] dark:bg-slate-800 text-slate-900 dark:text-white">{sort}</option>)}
            </select>
          </div>
        </motion.div>
